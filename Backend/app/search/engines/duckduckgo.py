...
class DuckDuckGoSearchEngine(BaseSearchEngine):
    """DuckDuckGo search without API requirements"""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(search_url, headers=headers, timeout=aiohttp.ClientTimeout(total=15)) as response:
                    if response.status == 200:
                        ...
                        print(f"📋 DuckDuckGo found {len(search_result_urls)} result URLs")
                        ...
                        error = f"DuckDuckGo search failed: HTTP {response.status}"
